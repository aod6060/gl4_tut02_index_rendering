...
static std::string g_caption = "Index Rendering";
...
// Index
static std::vector<uint32_t> b_index_list;
static uint32_t b_index_id = 0;
...
void demo_init() {
	...
	b_vertice_list.push_back(glm::vec3(-1.0f, 1.0f, 0.0f)); // 0
	b_vertice_list.push_back(glm::vec3(1.0f, 1.0f, 0.0f)); // 1
	b_vertice_list.push_back(glm::vec3(-1.0f, -1.0f, 0.0f)); // 2
	b_vertice_list.push_back(glm::vec3(1.0f, -1.0f, 0.0f)); // 3
	...
	b_colors_list.push_back(glm::vec4(1.0f, 1.0f, 0.0f, 1.0f));
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, b_index_id);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, b_index_list.size() * sizeof(uint32_t), b_index_list.data(), GL_DYNAMIC_DRAW);
...
void demo_render() {
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, b_index_id);
	glDrawElements(GL_TRIANGLES, b_index_list.size(), GL_UNSIGNED_INT, nullptr);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
